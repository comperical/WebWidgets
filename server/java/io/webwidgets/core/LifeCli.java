...
public class LifeCli
{
	...
	public static class CheckSinglePage extends ArgMapRunnable
	{
		
		public void runOp()
		{
			String target = _argMap.getStr("target");	
			File tfile = new File(target);
			
			ConfigCheck.checkServerPageConfig(tfile);
			;
	...
	public static class ConfigCheckUserWidget extends ArgMapRunnable
	{
		
		public void runOp()
		{
			File onefile = new File(LifeUtil.WIDGETS_DIR);			
			recurCheckFile(onefile);
		}
		
		private void recurCheckFile(File onefile)
		{
			if(onefile.getAbsolutePath().contains("WEB-INF"))
				{ return; }
			
			
			if(onefile.isDirectory())
			{
				for(File kid : onefile.listFiles())
					{ recurCheckFile(kid); }
				
				return;
			}
			
			if(onefile.getAbsolutePath().contains("dburfoot"))
			{
				List<String> skiplist = Util.listify("docview", "doccode", "chinese");
				
				for(String skip : skiplist)
				{
					if(onefile.getAbsolutePath().contains(skip))
					{ 
						Util.pf("Skipping check of dburfoot %s file\n", skip);
						return; 
					}
				}
				
				Util.pf("Checking dburfoot file!!!\n");
				// return;	
			}
			
			if(onefile.getAbsolutePath().endsWith(".jsp"))
			{
				Util.pf("Checking file %s\n", onefile);
				ConfigCheck.checkServerPageConfig(onefile);
			}
		}
	}	
	
	public static class CheckLocalWidgetConfig extends ArgMapRunnable
	{
		
		public void runOp()
		{
			File onefile = new File("/opt/userdata/lifecode/widgets");			
			recurCheckFile(onefile);
		}
